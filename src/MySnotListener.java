...
public class MySnotListener extends SnotBaseListener {
	...
	@Override public void enterParameter_list(SnotParser.Parameter_listContext ctx) { 
		for (TypeContext typeCtx : ctx.type()) {
			if (types_list.contains(typeCtx.ID().getText())) {
				System.out.println("Error, "+typeCtx.ID().getText()+" is alredy used");
			}
			if (!types_list.constais(typeCtx.ID().getText()) && !isPreDefinedType(typeCtx.type().getText())) {
				System.out.println("Error, "+typeCtx.type().getText()+" was not defined");
			}
		}
	}
